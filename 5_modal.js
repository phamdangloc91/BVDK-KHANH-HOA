// Hàm định dạng tiền tệ chuyên biệt cho Modal
let formatTienModal = function(val) { 
...
window.moChiTietDV = function(encodedMaDichVu, encodedMaTuongDuong, encodedTenDichVu) {
    ...
    let bhytHtml = '';
    if (giaDVInfo) {
        // DÙNG BỘ LỌC ĐỊNH DẠNG MỚI
        let formattedPrice = formatTienModal(giaDVInfo.giaMax);
...
window.moChiTiet = function(encodedMa, encodedTen, encodedPhanLoai, encodedQuyetDinh) {
    ...
    if (matchedPrices.length > 0) {
        ...
        matchedPrices.forEach(function(p) { 
            // DÙNG BỘ LỌC ĐỊNH DẠNG MỚI
            let formattedPrice = formatTienModal(p.giaMax); 
    ...
    if (matchedBVPrices.length > 0) {
        ...
        matchedBVPrices.forEach(function(p) {
            // DÙNG BỘ LỌC ĐỊNH DẠNG MỚI
            let gBHYT = formatTienModal(p.giaBHYT); if(gBHYT === 'Chưa có giá') gBHYT = '-';
            let gVP = formatTienModal(p.giaVienPhi); if(gVP === 'Chưa có giá') gVP = '-';
            let gYC = formatTienModal(p.giaYeuCau); if(gYC === 'Chưa có giá') gYC = '-';
            let gNN = formatTienModal(p.giaNuocNgoai); if(gNN === 'Chưa có giá') gNN = '-';
