...
// 🟢 ĐÃ SỬA: ĐỒNG BỘ QUYẾT ĐỊNH & PHÂN LOẠI TỪ PL1 SANG PL2 DỰA TRÊN CẢ "MÃ" VÀ "MÃ LIÊN KẾT"
window.enrichPL2 = function() {
    ...
    database.PL1.forEach(function(pl1Item) {
        if (!pl1Item) return;
        let maRaw1 = window.normalizeCodeFast(pl1Item.ma);
        let maRaw2 = window.normalizeCodeFast(pl1Item.maLienKet);
        
        let addMap = (m) => {
            let arr = m.split(';').filter(Boolean);
            arr.forEach(c => {
                if (!pl1Map.has(c)) pl1Map.set(c, []);
                pl1Map.get(c).push(pl1Item);
            });
        };
        if (maRaw1) addMap(maRaw1);
        if (maRaw2) addMap(maRaw2);
    });
    ...
    database.PL2.forEach(function(pl2Item) {
        ...
        let searchCodes = [];
        if (pl2Item.ma) searchCodes.push(...window.normalizeCodeFast(pl2Item.ma).split(';').filter(Boolean));
        if (pl2Item.maLienKet) searchCodes.push(...window.normalizeCodeFast(pl2Item.maLienKet).split(';').filter(Boolean));

        searchCodes.forEach(function(m) {
            if (pl1Map.has(m)) {
        ...
        if (qdSet.size > 0) pl2Item.quyetDinh = Array.from(qdSet).join('; ');
        else if (!pl2Item.quyetDinh || String(pl2Item.quyetDinh).trim() === "") pl2Item.quyetDinh = "Chưa phê duyệt";

        if (plSet.size > 0) pl2Item.phanLoai = Array.from(plSet).join(', ');
        else if (!pl2Item.phanLoai || String(pl2Item.phanLoai).trim() === "") pl2Item.phanLoai = "KPL";
    });
};
...
window.checkColorStatus = function(maHienThi) {
    let isHasBHYT = false; 
    let isHasBV = false;
    ...
    if (arrSearch.length > 0) {
        isHasBHYT = arrSearch.some(m => window.colorIndex.BHYT_Code.has(m));
        ...
        if (currentTabType === 'DTNH') {
            ...
            pageData.forEach(function(item, index) {
                ...
                if (currentTab === 'ICD10' || currentTabType === 'PHAC_DO') {
                        ...
                        if(tt === 'DA_PHE_DUYET') {
                            ...
                            if (currentUser && currentUser.role === 'khoa' && currentUser.tenKhoa === currentTab) {
                                ...
                                if(item.fileKhoa && tt !== 'CHUA_NOP') {
                                    ...
                                    fileHtml += `<a href="${item.fileKhoa}" target="_blank" style="font-size:12px; color:blue; display:inline-block; max-width: 180px; white-space: nowrap; overflow: hidden; text-overflow: margin-top:5px;" title="${dispNameAd}">📄 ${dispNameAd}</a><br>`;
                                }
                    ...
                    let colorRes = window.checkColorStatus(maHienThi);
                    let rowClass = "";
